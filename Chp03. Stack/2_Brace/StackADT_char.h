...
typedef char Element; //Changeable.
...
typedef struct {
 ...
 int isEmpty(Stack* pStack) { //return 1 empty, return 0 not empty
	 if (pStack->top < 0) {
		 return 1;
	 }
	 else {
 ...
 Stack* reverse_Stack(Stack* pStack) {
	 ...
	 for (int i = 0; i < pStack->size; i++) {
		 Element value = Pop(pStack);
		 Push(pStack_reverse, value);
	 }
